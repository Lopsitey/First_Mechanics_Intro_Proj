...
public class CharacterMovement : MonoBehaviour
{
    ...
    [SerializeField] private float
        m_JumpDelayTime = 0.2f; //the delay after a jump is executed - the minimum being ~ 0.15 due to the coyote time 

    [SerializeField]
    private float m_JumpBufferTime = 0.3f; //the buffer of time a jump is queued for - before hitting the ground

    [SerializeField]
    private float m_ApexGravity = 0.2f; //the level of gravity at the apex of a jump (1 is normal gravity)

    [SerializeField]
    private float m_ApexGravityTime = 0.2f; //the amount of time you experience reduced gravity at the apex of a jump
    ...
    private void FixedUpdate()
    {
        if (!m_CanJump || !m_GroundSensor.m_IsGrounded) //essentially if jumped or not grounded
        {
            ...
            if (m_CurrentState == JumpStates.Rising &&
                ALMOST_ZERO(m_RB.linearVelocityY)) //changes the state to apex if rising and vertical velocity is near 0
            {
